...
struct Stage2: ff_node_t<Item> {
    Item * svc(Item * task) {
        //cout << task->i << "\t";
        ParallelFor pf;
        pf.parallel_for(0,db.dim,[&](const int j) {
            double a, b, cr;
...
void Mapper(){
    //porque nwokers n pode ser usado diretamente no estagio 2 atravś de Database?
    ff_Pipe<Item> pipe(new Stage1(), new Stage2(), new Stage3());

    if (pipe.run_and_wait_end()<0) error("running Pipeline with Map"); 
}
...
int main(int argc, char **argv) {
    ...
    #if !defined(NO_DISPLAY)
        SetupXWindows(dim,dim,1,NULL,"Pipeline_Map Mandelbroot");
    #endif

    db.dim = dim;
    db.niter = niter;
    db.nworkers = nworkers;
    db.init_a = init_a;
    db.init_b = init_b;
    db.step = step;
