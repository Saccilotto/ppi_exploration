...
class stage2 {
    ...
    stage2(int _niter, double _init_a, int _dim, double _step):
        niter(_niter),
        init_a(_init_a),
        dim(_dim),
        step(_step)
...
void runPipe(int nthreads,int dim, double init_b, double step, double init_a, int niter) {  
    parallel_pipeline(
        ...
        make_filter<void,Item*>(
            filter::serial_in_order, stage1(dim, init_b, step))
        &
        make_filter<Item*,Item*>(
            filter::serial_out_of_order, stage2(niter, init_a, dim, step))
        &
